...
| FR-CG-2 | D-CG-5 dispatch → **`Settings.set()`** → queued settings-changed → `refresh_panels()` | L1 dispatch + the worker→GUI hop; L2 write class; L3 | **L1 ✅** (dispatch half) `V-CG-13`, `V-CF-16`; the queued hop is session 3 |
| FR-CG-3 | D-CG-5 undo journal, **incl. `update_memory` and reverse-order restore** | L1; L3 | **L1 ✅** `V-CG-40`…`V-CG-45` |
...
| FR-CG-9 | D-CG-1 **job object (§8.1)** + state-file reap | L1 reap logic; **L3 exit, `TerminateProcess`, and Task-Manager-kill process audits** | **L1 ✅** `V-CG-48`…`V-CG-53` |
...
| FR-CG-11 | **D-CG-13 `Settings.set()`** + D-CG-3 repair loop (incl. truncation class) | L1 forced-rejection at write time + forced-truncation; L2 refusal class | **L1 ✅** `V-CF-15`, `V-CF-16`, `V-CG-13`, `V-CG-25`, `V-CG-36` |
