...
| `V-M-96` | **The release split** | The distribution rebuilt as two archives and both extracted into one folder, entry by entry | ✅ pass — **8524 + 28 = 8552**, the single archive's count exactly, with **no overlap**, no runtime file in the application archive and no application file in the runtime one. Extracted together they produce 8552 files with every size matching and `app/llama/` beside `app/ptt/`; on disk the pair is **2090.35 MB**, which is what the one archive weighed. 71 % and 31 % of GitHub's 2 GiB per-asset limit, which the build now enforces rather than discovering at the end of an upload (#55) |
...
### 5.5 v3.0 release preparation · 2026-08-29

Between the acceptance pass and the release. Three items, and the first is the
one the other two rest on.

| ID | Test | Result |
|---|---|---|
| `V-M-96` | **Can llama-server run on the cuBLAS that ships for Whisper?** Three arms — its own cuBLAS, none at all, and CTranslate2's on `PATH` — each measured by VRAM and decode rate rather than by whether it answered | ✅ pass. **8395 MiB / 25.3 tok/s** on its own, **0 MiB / 5.7 tok/s** with none, **8395 MiB / 25.7 tok/s** on CTranslate2's — identical to baseline to the megabyte. The middle arm is the point: `ggml-cuda.dll` statically imports cuBLAS, and llama.cpp **skips a backend it cannot load and runs on CPU without complaint**, so all three arms "started and generated tokens" and the first version of this experiment concluded the opposite of the truth (#56) |
| `V-M-97` | **The release candidate, extracted and run.** 1.668 GiB, one archive, `app/llama/` carrying no cuBLAS at all | ✅ pass, nine checks. The extracted root offers exactly one executable; the runtime is 26 files with both licences and no cuBLAS; the copy it borrows is in the same download. The shipped application reached `ready` and **held 8409 MiB** — the number that distinguishes a working GPU from the silent fallback. No traceback, no `WRONG THREAD`, and `TerminateProcess` still took the child with it. **One earlier run of this was discarded**: a stray `llama-server` was holding 10.7 GB, so the copy under test could not fully load and the job-object check was watching a process that was never its child. The v3-7 driver refuses to start in that state and this one did not; it does now |
| `V-M-98` | **A fresh install with the `_internal/` layout**, verified against the installation the user made themselves from this archive rather than a synthetic one | ✅ pass. Both shortcuts target `…\ptt_dictate\_internal\run_tray.bat` with the run-as-administrator byte set and the working directory at the installation root; the installed `app/llama/` is the deduplicated 26 files with both licences; the knowledge pack is present; and the application ran elevated and recorded a real dictation. The elevation wrapper in `install.bat` is still not re-checked — it raises a UAC dialog no script can accept, and `V-M-74` covered it |

**What changed in the distribution, and why.** The archive was 2.041 GiB and a
GitHub release asset must be under 2 GiB. It was briefly two archives split at
`app/llama/`; that was abandoned, because a user extracts a zip, looks for
something called install and double-clicks it, and a second download they have
to know about is a step most people will not take. It is one archive again
because it stopped shipping cuBLAS twice — #55 — and one file in the root
because two files both displayed as `install` is a coin toss — #57.
...
| **A broken CUDA install gets a Concierge that is four times slower and says nothing** | Found while measuring `V-M-96`: `ggml-cuda.dll` statically imports cuBLAS, and llama.cpp **skips a backend it cannot load and runs on CPU without complaint** — 5.7 tok/s against 25.3, holding no VRAM. Nothing in the application notices or reports it. `server.py` already drains llama-server's stderr into the log, which names the backend and the CUDA buffer size, so the fix is to read what is already there and say so in the panel rather than to add a probe | next session |
